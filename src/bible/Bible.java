...
//								log.info("... from " + url);
                ...
                Writer fw = new OutputStreamWriter(new FileOutputStream(chapterFile), StandardCharsets.UTF_8);
                fw.write(chapterPage);
                fw.close();
              }
            }
          }
  ...
  private String downloadChapter(InputStream is)
  {
...
//    Reader isr = new InputStreamReader(is, StandardCharsets.ISO_8859_1);
    ...
    try
    {
      ...
      int charsRead;
      while ((charsRead = isr.read(chunk)) != -1)
      {
        page.append(chunk, 0, charsRead);
      }
      isr.close();
    }
    catch (IOException e)
